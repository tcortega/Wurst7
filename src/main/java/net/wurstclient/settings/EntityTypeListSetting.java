...
public class EntityTypeListSetting extends Setting {
    ...
    public EntityTypeListSetting(String name, WText description, String... entities) {
    ...
    public void add(EntityType<?> entityType) {
        ...
        Collections.sort(entityTypes, Comparator.comparing(EntityUtils::getName));
    ...
    public void resetToDefaults() {
        ...
        entityTypes.addAll(
                Arrays.stream(defaultTypes).parallel()
                        .map(s -> Registries.ENTITY_TYPE.get(new Identifier(s)))
                        .filter(Objects::nonNull).toList()
        );
        ...
        try {
            ...
            JsonUtils.getAsArray(json).getAllStrings().parallelStream()
                    ...
                    .sorted().forEachOrdered(name -> {
                        EntityType<?> entityType = Registries.ENTITY_TYPE.get(new Identifier(name));
                        entityTypes.add(entityType);
                        entityTypeNames.add(name);
                    });
